// src/app/api/org/product-custom-lists/route.ts
...
export async function GET() {
  ...
  const { data: lists } = await auth.adminClient.from('product_custom_lists').select('*').eq('org_id', auth.orgId).order('sort_order').order('created_at')
  const { data: options } = await auth.adminClient.from('product_custom_list_options').select('*').eq('org_id', auth.orgId).order('sort_order').order('created_at')
...
export async function POST(req: Request) {
  ...
  const { data, error } = await auth.adminClient.from('product_custom_lists')
