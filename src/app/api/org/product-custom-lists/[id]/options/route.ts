// src/app/api/org/product-custom-lists/[id]/options/route.ts
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await auth.adminClient.from('product_custom_list_options')
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!value?.trim()) return NextResponse.json({ error: 'Value is required' }, { status: 400 })
  const { error } = await auth.adminClient.from('product_custom_list_options')
