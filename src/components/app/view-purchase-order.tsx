...
export default function ViewPurchaseOrder({
  ...
  const filteredSuppliers = contacts.filter(s =>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, overflow: 'hidden' }} onClick={() => { setTermsOpen(false); setLocationOpen(false); setSupplierOpen(false) }}>
          ...
          <div className="npo-card">
            ...
            {mode === 'view' ? (
              ...
              <div style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
                <input
                  ...
                  value={supplierSearch || editSupplierName}
            ...
            <div className="modal-field" style={{ marginTop: 12 }}>
              ...
              {mode === 'view' ? (
                ...
                <input className="modal-input" value={editRef} onChange={e => setEditRef(e.target.value)} placeholder="Supplier's reference number" style={{ background: 'var(--white)' }} />
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginTop: 4 }}>
            ...
            <div className="modal-field" style={{ gridColumn: 'span 3' }}>
              ...
              {mode === 'view' ? (
                ...
                <textarea className="modal-input" value={editNotes} onChange={e => setEditNotes(e.target.value)} rows={2} style={{ resize: 'vertical', height: 60, lineHeight: 1.5, background: 'var(--white)' }} placeholder="Internal notes…" />
              )}
            </div>
          </div>
        </div>
          ...
          <div style={{ overflowX: 'auto', margin: '0 -20px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
              ...
              <tbody>
                {lines.length === 0 && (
                  <tr>
                    <td colSpan={8} style={{ textAlign: 'center', padding: '24px 0', color: 'var(--gray-400)', fontSize: 13 }}>No line items.</td>
                  </tr>
                )}
                {lines.map(l => {
                  ...
                  return (
                    <tr key={l.id} style={{ borderBottom: '1px solid var(--gray-100)' }}>
                      ...
                      <td className="li-td" style={{ textAlign: 'right', color: remaining > 0 ? 'var(--danger)' : '#059669', fontWeight: 600 }}>{remaining}</td>
      ...
      <div style={{ background: 'var(--white)', borderTop: '1px solid var(--gray-100)', padding: '14px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 -4px 16px rgba(0,0,0,0.06)', flexShrink: 0 }}>
        {mode === 'view' ? (
          ...
          <>
            <button onClick={cancelEdit} className="btn btn-outline" style={{ height: 38 }}>Cancel</button>
